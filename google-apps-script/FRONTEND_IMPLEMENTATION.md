...
## Donation Logging System - UI Changes Required

---

## 🎯 Objective

Update the emergency request system frontend to work with the new donation logging backend that tracks partial donations.

---

## 📝 Changes Required

### **1. Update Event Listener (Line ~55-65)**
...
### **2. Update `createRequestCard()` Function (Line ~273-480)**

#### **A. Add Units Calculation (After line ~314)**

Add these lines after the patient age formatting:
...
#### **B. Update Button Variables (Replace lines ~295-311)**

Replace the close button variables with log donation button variables:

```javascript
// Determine Log Donation button state
let logDonationButtonClass = 'btn-donation btn-flex log-donation-btn';
let logDonationButtonText = 'Log Donation';
let logDonationButtonDisabled = '';

// Check if request is already closed
if (requestStatus === 'Closed' || (storedState && storedState.closeStatus === 'closed')) {
    logDonationButtonClass = 'btn-closed btn-flex log-donation-btn';
    logDonationButtonText = 'Closed';
    logDonationButtonDisabled = 'disabled';
}
```

#### **C. Update Grid HTML (Replace lines ~440-457)**

Replace the grid section to show units fulfilled/remaining:
...
#### **D. Update Button HTML (Replace close button around line ~471)**

Replace the close button with log donation button:

```html
<button class="${logDonationButtonClass} mobile-button" ${logDonationButtonDisabled}>
    <svg class="w-5 h-5 mr-2 inline" fill="currentColor" viewBox="0 0 20 20">
        <path fill-rule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM6.293 6.707a1 1 0 010-1.414l3-3a1 1 0 011.414 0l3 3a1 1 0 01-1.414 1.414L11 5.414V13a1 1 0 11-2 0V5.414L7.707 6.707a1 1 0 01-1.414 0z" clip-rule="evenodd" />
    </svg>
    <span>${logDonationButtonText}</span>
</button>
```

---

### **3. Create New `logDonation()` Function**

Replace the old `closeRequest()` function (line ~719-847) with this new function:
...
### **4. Create New `showDonationPopup()` Function**

Replace the old `showDonorInfoPopup()` function with this new one:
...
- [ ] "Log Donation" button appears instead of "Close"
- [ ] Units fulfilled/remaining display correctly
- [ ] Clicking "Log Donation" shows the new popup
- [ ] Can select units (1, 2, 3, or custom)
- [ ] Can select donor type (Relative/Donor/Other)
- [ ] Donor details form appears when "Donor" selected
- [ ] Closure reason field appears when "Other" selected
- [ ] Validation works (units > 0, <= remaining, donor name required, etc.)
- [ ] Donation logs successfully to backend
- [ ] Card updates to show new units fulfilled
- [ ] Request auto-closes when all units fulfilled
- [ ] Error messages display for over-donation attempts

---

## 📌 Notes

1. **File to Edit:** `scripts/emergency_request_system.js`
2. **Backup First:** Make a backup before editing
3. **Test Locally:** Test all changes before deploying
4. **Backend Must Be Deployed:** Ensure the Google Apps Script is deployed first

---

**Ready to implement?** Follow the steps above in order, testing each change as you go.
