...
function formatRequestDataForCopy(requestData) {
    ...
    return [
        ...
        formatField('Units Required', requestData.unitsRequiredText || requestData.unitsRequired),
    ...
    return new Promise((resolve) => {
        ...
        modalContent.innerHTML = `
            ...
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <button id="whatsappShareBtn" style="display: flex; align-items: center; padding: 16px; border: 1px solid #d1d5db; border-radius: 12px; background-color: #25D366; color: white; font-weight: 500; cursor: pointer; transition: all 0.2s ease;">
                ...
                <button id="downloadPosterBtn" style="display: flex; align-items: center; padding: 16px; border: 1px solid #d1d5db; border-radius: 12px; background-color: #DC2626; color: white; font-weight: 500; cursor: pointer; transition: all 0.2s ease;">
        ...
        const whatsappBtn = modalContent.querySelector('#whatsappShareBtn');
        const downloadBtn = modalContent.querySelector('#downloadPosterBtn');
        ...
        whatsappBtn.addEventListener('mouseenter', () => {
            whatsappBtn.style.backgroundColor = '#1faa59';
        });
        ...
        downloadBtn.addEventListener('mouseenter', () => {
            downloadBtn.style.backgroundColor = '#b91c1c';
        });
        ...
        whatsappBtn.addEventListener('click', () => {
            const message = generateWhatsAppMessage(requestData);
        ...
        downloadBtn.addEventListener('click', async () => {
            try {
                ...
                await generatePoster(requestData);

                showSuccessMessage('Poster downloaded successfully!');
                modal.remove();
                resolve(true);
            } catch (error) {
                console.error('Error generating poster:', error);
                showSuccessMessage('Error generating poster. Please try again.');
                downloadBtn.innerHTML = `
                    <svg style="width: 24px; height: 24px; margin-right: 12px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
                    </svg>
...
function formatRequestDataForTwitter(requestData) {
    ...
    const bloodType = requestData.bloodType || '';
    const units = requestData.unitsRequiredText || requestData.unitsRequired || '';
